...
function run(vertexShaderText, fragmentShaderText, model, image) {
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, normalsBuffer);
    const vertNormal = gl.getAttribLocation(program, 'vertNormal');
    gl.vertexAttribPointer(
        vertNormal, // the attribute location
        3, // number of elements per attribute
        gl.FLOAT, // the type of elements
        true, // has a unit length of 1 (normalized)
        3 * Float32Array.BYTES_PER_ELEMENT, // size of an individual vertex
        0 // offset from the beginning of a single vertex to this attribute
    );
    ...
    gl.useProgram(program);
    const ambientLightIntensity = gl.getUniformLocation(program, 'ambientLightIntensity');
    const sunLightIntensity = gl.getUniformLocation(program, 'sun.color');
    const sunLightDirection = gl.getUniformLocation(program, 'sun.direction');

    gl.uniform3f(ambientLightIntensity, 0.2, 0.2, 0.2);
    gl.uniform3f(sunLightDirection, 3.0, 4.0, -2.0);
    gl.uniform3f(sunLightIntensity, 0.9, 0.9, 0.9);
